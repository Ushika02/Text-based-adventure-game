...
public class Storyline {
	...
	int swordCount = 0, axeCount = 0, goblinTeeth = 0, orgreClaw = 0, wraithCloth = 0, fireC = 0, frostC = 0, lightC = 0;

	//Initializing prices of items for shop
	...
	public void heal() { // increaing playerHp when healing
		if (character == "healer" && turn > 3) { // character should be healer and heal is available after every attacks done by player
			playerHP += healer.getHealingPower(); // call method from healer class
			if (playerHP > 100) {
				playerHP = 100;
			}
			ui.hpLabelNumber.setText("" + playerHP);
	...
	public void saveGame() {
		...
		catch (Exception e) { // handle various types of errors that can occur during the execution of a program
			e.printStackTrace();
	...
	public void river1() { // going to river for the 1st time - side quest
		...
		ui.mainTextArea.setText("You went down to a nearby river to rest and bath, when you see that the water has been poisoned.\n\nPeople in surrounding camps seem to be infected and in agony.");
	...
	public void cave() {
		...
		if (torch == 0) {
			ui.mainTextArea.setText("Leaving the torch, you entered the dark cave. Unable to avoid the obstacles, you take damage.\n\nDamage inccured = 10 HP\n\nEventually you find yourself at a dead end.\n\nBefore you, two towering statues stand guard.");
			playerHP -= 15;
			ui.hpLabelNumber.setText("" + playerHP);
	...
	public void attackMonster(String name) {
		if (name == "wolf") { // in cave - puzzle failed
			...
			if (dragon.getHP() <= damage) {
				ui.mainTextArea.setText("You attack the dragon, giving it " + dragonHP + " damage, defeating it.\n\nYou free the girl and take her back to town.");
	...
	public void goldchest() {
		...
		if (reward instanceof Gold) { // obtain gold from chest
			...
			ui.goldLabelNumber.setText("" + gold);  // update gold in player panel
